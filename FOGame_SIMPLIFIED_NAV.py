...
def suggested_player_card_link(row: pd.Series, slot: str) -> str:
    """Clickable HTML card for the one-row suggested-player rail."""
    player_name = str(row.get("Player", ""))
    href = f"?add_slot={quote(str(slot))}&add_player={quote(player_name)}"
    return (
        f'<a class="suggest-card-link" href="{href}">'
        f'<div class="suggest-card">'
        f'<div class="suggest-pos">{row["Pos"]}</div>'
        f'<div class="suggest-img-wrap">{player_headshot_html(row)}</div>'
        f'<div class="suggest-name">{player_name}</div>'
        f'<div class="suggest-meta">{row["Team"]} • {row["Pos"]}</div>'
        f'<div class="suggest-salary">{money(row["Salary"])}</div>'
        f'<div class="suggest-add-mini">Add</div>'
        f'</div>'
        f'</a>'
    )
