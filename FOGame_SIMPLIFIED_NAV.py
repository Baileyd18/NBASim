...
def render_roster_list(show_remove: bool = False):
    ...
    for slot in active_roster_slots:
        if slot in st.session_state.roster:
            ...
            add_col, card_col = st.columns([0.16, 0.84], gap="small")

            with add_col:
                if st.button("＋", key=f"quick_add_{slot}", use_container_width=True):
                    st.session_state.quick_add_slot = slot
                    st.rerun()

            with card_col:
                st.markdown(
                    f"""
                    <div class="player-row-card" style="opacity:.72;">
                        <div class="slot-pill">{slot_short(slot)}</div>
                        <div>
                            <div class="player-name-mobile">Empty Slot</div>
                            <div class="player-role-mobile">Tap + to search players for this role</div>
                        </div>
                        <div class="player-right-mobile">
