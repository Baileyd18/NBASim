...
def build_dialog_player_pool(slot: str, team_filter: str, position_filter: str, salary_filter: str, search_query: str) -> pd.DataFrame:
    """
    Dialog-specific player search.
    Fixes mobile search issues by:
    - not hiding low-minute players
    - matching normalized names so accents/punctuation do not block results
    - filtering by Team / Position / Salary separately
    - prioritizing exact/name matches before stat sorting
    """
    player_pool = df.copy()
    ...
    if position_filter != "Pos":
        player_pool = player_pool[player_pool["Pos"] == position_filter]

    if salary_filter != "Salary":
    ...
    if q:
        ...
        profile_norm = player_pool.get("PlayerProfile", pd.Series([""] * len(player_pool), index=player_pool.index)).astype(str).str.lower()
        ...
        if len(player_pool) > 0:
            ...
            player_pool.loc[player_pool["Player"].apply(normalize_name_for_match) == q_norm, "Search_Priority"] = 0
            player_pool.loc[player_pool["Player"].apply(normalize_name_for_match).str.startswith(q_norm, na=False), "Search_Priority"] = 1
            player_pool.loc[player_pool["Player"].apply(normalize_name_for_match).str.contains(q_norm, na=False), "Search_Priority"] = 2
            player_pool = player_pool.sort_values(["Search_Priority", "PTS", "MP"], ascending=[True, False, False])
    else:
        # Suggested/default list: best available players, not limited to the first 100 too early.
        player_pool = player_pool.sort_values(["PTS", "Impact_Score", "MP"], ascending=[False, False, False])
...
def open_player_search_dialog(slot: str):
    ...
    # Compact three-filter row: replaces the bulky top search field.
    f_team, f_pos, f_salary = st.columns(3)
    with f_team:
        team_filter = st.selectbox(
            "Team",
            ["Team"] + sorted(df["Team"].dropna().astype(str).unique().tolist()),
    ...
    with f_pos:
        position_filter = st.selectbox(
            "Position",
            ["Pos"] + sorted(df["Pos"].dropna().astype(str).unique().tolist()),
            label_visibility="collapsed",
    ...
    with f_salary:
        salary_filter = st.selectbox(
            "Salary",
            ["Salary", "Under $5M", "$5M-$15M", "$15M-$30M", "$30M+"],
            label_visibility="collapsed",
    ...
    search_query = st.text_input(
        "Choose player",
        placeholder="Search player name, team, position, or archetype",
        key=f"dialog_search_{slot}"
    )

    player_pool = build_dialog_player_pool(slot, team_filter, position_filter, salary_filter, search_query)
    ...
    # Five suggested player cards above the main selected-player preview.
    suggested_players = player_pool.head(5).copy()
    st.markdown("<div class='small-muted' style='margin-top:8px;margin-bottom:7px;'>Suggested players</div>", unsafe_allow_html=True)
    suggestion_cols = st.columns(5)

    if f"dialog_selected_player_{slot}" not in st.session_state:
        st.session_state[f"dialog_selected_player_{slot}"] = suggested_players.iloc[0]["Player"]

    available_names = player_pool["Player"].tolist()
    if st.session_state[f"dialog_selected_player_{slot}"] not in available_names:
        st.session_state[f"dialog_selected_player_{slot}"] = available_names[0]

    for i, (_, srow) in enumerate(suggested_players.iterrows()):
        with suggestion_cols[i]:
            label = f"{srow['Player']}\n{money(srow['Salary'])}"
            if st.button(label, use_container_width=True, key=f"suggest_{slot}_{i}_{srow['Player']}"):
                st.session_state[f"dialog_selected_player_{slot}"] = srow["Player"]

    selected_player_name = st.selectbox(
        "Search Results",
        available_names[:300],
        index=available_names[:300].index(st.session_state[f"dialog_selected_player_{slot}"])
        if st.session_state[f"dialog_selected_player_{slot}"] in available_names[:300]
        else 0,
        key=f"dialog_player_select_{slot}",
        label_visibility="collapsed"
    )
    st.session_state[f"dialog_selected_player_{slot}"] = selected_player_name
