            ...
            with suggestion_cols[i]:
                st.markdown(
                    ...
                    <div class="suggest-card">
                        <div class="suggest-pos">{srow['Pos']}</div>
                        <div class="suggest-img-wrap">{player_headshot_html(srow)}</div>
                        <div class="suggest-name">{srow['Player']}</div>
                        <div class="suggest-meta">{srow['Team']} • {srow['Pos']}</div>
                        <div class="suggest-salary">{money(srow['Salary'])}</div>
                    </div>
                    """,
                    unsafe_allow_html=True
                )
                if st.button("Add", use_container_width=True, key=f"suggest_add_{slot}_{i}_{srow['Player']}"):
                    add_dialog_player(srow, slot)
                    st.rerun()
    else:
        st.info("No suggested players available with these filters.")

    all_left, sort_right = st.columns([0.55, 0.45], vertical_alignment="end")
    with all_left:
