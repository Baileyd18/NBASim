...
PRESET_ROSTERS = {
    ...
    'Boston': [
        ...
        ('Bench 4', 'Dalano Banton'),
        ('Bench 5', 'Hugo Gonzalez'),
        ('Two-Way 1', 'Luka Garza'),
        ('Two-Way 2', 'Max Shulga'),
        ('Bench 6', 'Ron Harper Jr.'),
        ('Bench 7', 'Amari Williams'),
    ],
    'Brooklyn': [
    ...
    'Dallas': [
        ...
        ('Bench 5', 'Ryan Nembhard'),
        ('Two-Way 1', 'Caleb Martin'),
        ('Two-Way 2', 'Marvin Bagley III'),
        ('Bench 6', 'John Poulakidas'),
        ('Bench 7', 'Moussa Cisse'),
    ],
    'Denver': [
    ...
    'Golden State': [
        ...
        ('Bench 5', 'Charles Bassey'),
        ('Two-Way 1', 'LJ Cryer'),
        ('Two-Way 2', 'Malevy Leons'),
        ('Bench 6', 'Quinten Post'),
        ('Bench 7', 'Nate Williams'),
    ],
    'Houston': [
    ...
    'LA Clippers': [
        ...
        ('Bench 6', 'Norchad Omier'),
        ('Bench 7', 'TyTy Washington Jr.'),
    ],
    'LA Lakers': [
    ...
    'Miami': [
        ...
        ('Bench 6', 'Myron Gardner'),
        ('Bench 7', 'Jahmir Young'),
    ],
    'Milwaukee': [
        ...
        ('Bench 6', 'Thanasis Antetokounmpo'),
        ('Bench 7', 'Gary Harris'),
    ],
    'Minnesota': [
        ...
        ('Bench 6', 'Julian Phillips'),
        ('Bench 7', 'Rocco Zikarsky'),
    ],
    'New Orleans': [
    ...
    'Oklahoma City': [
        ...
        ('Bench 5', 'Jaylin Williams'),
        ('Two-Way 1', 'Isaiah Joe'),
        ('Two-Way 2', 'Aaron Wiggins'),
        ('Bench 6', 'Branden Carlson'),
        ('Bench 7', 'Nikola Topic'),
    ],
    'Orlando': [
    ...
    'Phoenix': [
        ...
        ('Bench 4', 'Jamaree Bouyea'),
        ('Bench 5', 'Haywood Highsmith'),
        ('Two-Way 1', 'Ryan Dunn'),
    ...
    'San Antonio': [
        ...
        ('Bench 4', 'Luke Kornet'),
        ('Bench 5', 'Jordan McLaughlin'),
        ('Two-Way 1', 'Mason Plumlee'),
        ('Two-Way 2', 'David Jones Garcia'),
        ('Bench 6', 'Lindy Waters III'),
        ('Bench 7', 'Kelly Olynyk'),
    ],
    'Toronto': [
    ...
    'Utah': [
        ...
        ('Bench 5', 'Elijah Harkless'),
        ('Two-Way 1', 'John Konchar'),
        ('Two-Way 2', 'Bez Mbeng'),
        ('Bench 6', 'Kevin Love'),
        ('Bench 7', 'Blake Hinson'),
    ],
    'Washington': [
        ...
        ('Bench 4', 'Tristan Vukcevic'),
        ('Bench 5', 'Sharife Cooper'),
        ('Two-Way 1', 'Jaden Hardy'),
        ('Two-Way 2', 'Cam Whitmore'),
        ('Bench 6', "D'Angelo Russell"),
        ('Bench 7', 'Jamir Watkins'),
    ],
}
...
def open_player_search_dialog(slot: str):
    st.markdown(f"### Add player to **{slot}**")
    ...
    player_pool = player_pool[~player_pool["Player"].isin(selected_players)]

    if search_query:
        player_pool = player_pool[
            player_pool["Player"].str.contains(search_query, case=False, na=False) |
            player_pool["Team"].str.contains(search_query, case=False, na=False) |
            player_pool["Pos"].str.contains(search_query, case=False, na=False)
        ]

    player_pool = player_pool[player_pool["MP"] >= 10]
    player_pool = player_pool.sort_values("PTS", ascending=False).head(100)

    if len(player_pool) == 0:
        st.warning("No players found.")
        if st.button("Close", use_container_width=True, key=f"dialog_close_empty_{slot}"):
            st.rerun()
        return

    selected_player_name = st.selectbox(
        "Choose player",
        player_pool["Player"].tolist(),
        key=f"dialog_player_select_{slot}"
    )

    selected_player = player_pool[player_pool["Player"] == selected_player_name].iloc[0]

    st.markdown(
        f"""
        <div class="dialog-player-card">
            {player_headshot_html(selected_player)}
            <div>
                <div class="player-name-mobile">{selected_player['Player']}</div>
                <div class="player-role-mobile">
                    {selected_player['Team']} • {selected_player['Pos']} • {money(selected_player['Salary'])}<br>
                    {selected_player['PTS']:.1f} PPG • {selected_player['AST']:.1f} APG • {selected_player['TRB']:.1f} RPG<br>
                    {pct(selected_player['TS%'])} TS • {pct(selected_player['3P%'])} 3P
                </div>
            </div>
        </div>
        """,
    ...
    with c1:
        if st.button("Add Player", type="primary", use_container_width=True, key=f"dialog_add_{slot}"):
            player = selected_player.to_dict()
            player["Slot"] = slot
            ...
            st.markdown(
                ...
                <div class="player-row-card">
                    ...
                    <div class="player-right-mobile">
                        <div class="player-ovr-mobile">FIT {float(quality):.0f}</div>
