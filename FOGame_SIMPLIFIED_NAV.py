...
# HTML roster cards use query params so the plus box can stay inside the card
# while still opening a real Streamlit search/add panel.
try:
    query_slot = st.query_params.get("quick_add_slot", None)
    if query_slot in ROSTER_SLOTS:
        st.session_state.quick_add_slot = query_slot
except Exception:
    pass
...
def render_topbar():
    st.markdown(
        ...
        <div class="app-topbar">
            ...
            <div style="color:#94a3b8;font-size:26px;">☰</div>
        </div>
        """,
        unsafe_allow_html=True
    )
        ...
        if slot in st.session_state.roster:
            ...
            st.markdown(
                ...
                <div class="player-row-card">
                    <div class="slot-pill">{slot_short(slot)}</div>
                    <div class="avatar-circle">{initials(p['Player'])}</div>
                    <div>
            ...
            if show_remove:
                if st.button(f"Remove {p['Player']}", key=f"remove_{slot}_{p['Player']}", use_container_width=True):
                    del st.session_state.roster[slot]
                    st.session_state.reports = {}
                    st.rerun()
        else:
            ...
            st.markdown(
                f"""
                <a class="empty-slot-link" href="?quick_add_slot={slot_url}">
                    <div class="player-row-card empty-player-card">
                        <div class="slot-pill">{slot_short(slot)}</div>
                        <div class="plus-box">+</div>
                        <div>
                            <div class="player-name-mobile">Empty Slot</div>
                            <div class="player-role-mobile">Add a player to fill this role</div>
                        </div>
                        <div class="player-right-mobile">
                            <div class="player-ovr-mobile">OPEN</div>
                            <div class="player-salary-mobile">--</div>
                        </div>
                        <div class="chev-mobile">›</div>
                    </div>
                </a>
...
def render_quick_add_panel():
    """Shows a search/add panel after a user taps + on an empty roster slot."""
    if not st.session_state.get("quick_add_slot"):
        return

    quick_slot = st.session_state.quick_add_slot

    st.markdown(
        f'<div class="section-heading-mobile">Add Player to {quick_slot}</div>',
        unsafe_allow_html=True
    )

    quick_search = st.text_input(
        "Search player",
        key=f"quick_search_{quick_slot}",
        placeholder="Type a player, team, or position"
    )

    quick_pool = df.copy()
    selected_players = [v["Player"] for v in st.session_state.roster.values()]
    quick_pool = quick_pool[~quick_pool["Player"].isin(selected_players)]

    if quick_search:
        quick_pool = quick_pool[
            quick_pool["Player"].str.contains(quick_search, case=False, na=False) |
            quick_pool["Team"].str.contains(quick_search, case=False, na=False) |
            quick_pool["Pos"].str.contains(quick_search, case=False, na=False)
        ]

    quick_pool = quick_pool[quick_pool["MP"] >= 10].sort_values("PTS", ascending=False)

    if len(quick_pool) == 0:
        st.warning("No available players found.")
        if st.button("Cancel", use_container_width=True, key=f"quick_cancel_empty_{quick_slot}"):
            st.session_state.quick_add_slot = None
            try:
    ...
    quick_player_name = st.selectbox(
        "Choose player",
        quick_pool["Player"].head(150).tolist(),
        key=f"quick_player_{quick_slot}"
    )

    quick_player = quick_pool[quick_pool["Player"] == quick_player_name].iloc[0]

    st.markdown(
        f"""
        <div class="player-row-card" style="margin:14px 0;">
            <div class="slot-pill">{quick_player['Pos']}</div>
            <div class="avatar-circle">{initials(quick_player['Player'])}</div>
            <div>
                <div class="player-name-mobile">{quick_player['Player']}</div>
                <div class="player-role-mobile">
                    {quick_player['Team']} • {quick_player['PPG']:.1f} PPG • {quick_player['APG']:.1f} APG • {quick_player['RPG']:.1f} RPG
                </div>
            </div>
            <div class="player-right-mobile">
                <div class="player-ovr-mobile">BPM {float(quick_player['BPM']):.1f}</div>
                <div class="player-salary-mobile">{money(quick_player['Salary'])}</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    with c_add:
        if st.button("Add to Slot", type="primary", use_container_width=True, key=f"quick_confirm_{quick_slot}"):
            add_player_to_slot(quick_player, quick_slot)
            st.session_state.quick_add_slot = None
            try:
                st.query_params.clear()
            except Exception:
    ...
    with c_cancel:
        if st.button("Cancel", use_container_width=True, key=f"quick_cancel_{quick_slot}"):
            st.session_state.quick_add_slot = None
            try:
                st.query_params.clear()
            except Exception:
    ...
    if not open_slots:
        ...
        st.markdown(
            ...
            <div class="player-row-card" style="margin:14px 0;">
                ...
                <div class="avatar-circle">{initials(preview['Player'])}</div>
                <div>
