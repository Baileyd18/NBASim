...
/* Horizontal suggested player rail. This prevents Streamlit columns from stacking on mobile. */
.suggest-rail {
    display: flex;
    flex-direction: row;
    gap: 10px;
...
.suggest-rail::-webkit-scrollbar {
    height: 5px;
}

.suggest-rail::-webkit-scrollbar-thumb {
...
def dialog_sort_pool(player_pool: pd.DataFrame, sort_label: str) -> pd.DataFrame:
    ...
    # Default: PPG
    return player_pool.sort_values(["PTS", "AST", "TRB", "MP"], ascending=[False, False, False])
...
def build_dialog_player_pool(slot: str, team_filter: str, position_filter: str, salary_filter: str, search_query: str, sort_label: str) -> pd.DataFrame:
    ...
    if str(search_query or "").strip():
        ...
        if "Search_Priority" in sorted_pool.columns:
            sorted_pool = sorted_pool.sort_values(["Search_Priority", sorted_pool.columns[0]], ascending=[True, True])
        return sorted_pool
...
def build_suggested_players_for_slot(slot: str, team_filter: str, position_filter: str, salary_filter: str) -> tuple[pd.DataFrame, str]:
    """Suggest players by starter position or bench/two-way roster need."""
    ...
    target_positions = slot_target_positions(slot)

    def score_row(row):
        score = 0.0
        pos_match = player_matches_any_position(row, target_positions)

        score += float(row.get("Impact_Score", 0)) * 0.35
        score += float(row.get("PTS", 0)) * 1.7
        score += float(row.get("AST", 0)) * 1.1
        score += float(row.get("TRB", 0)) * 0.9
        score += float(row.get("TS%", 0)) * 22
        score += float(row.get("MP", 0)) * 0.25

        if pos_match:
            score += 35

        # Starter slots should heavily favor exact position fit.
        if slot.startswith("Starting") and not pos_match:
            score -= 45

        # Two-way slots should lean cheaper/developmental.
        if "Two-Way" in slot:
            ...
            if salary <= 5_000_000:
                ...
                score -= 35
            score += max(0, 22 - float(row.get("MP", 0))) * 0.45

        # Bench slots fill need, then reward practical rotation players.
        if slot.startswith("Bench"):
            if pos_match:
                score += 20
            if float(row.get("Salary", 0)) <= 12_000_000:
                score += 7
            if float(row.get("BPM", 0)) >= 0:
                score += 6

        return score

    pool = pool.copy()
    pool["Suggestion_Score"] = pool.apply(score_row, axis=1)
    suggested = pool.sort_values(["Suggestion_Score", "PTS", "MP"], ascending=[False, False, False]).head(5)

    if slot.startswith("Starting"):
        reason = f"Suggested because this slot needs a true {target_positions[0]}."
    elif "Two-Way" in slot:
        reason = "Suggested for low-cost depth and developmental value."
    else:
        reason = "Suggested based on your bench position needs: " + ", ".join(target_positions) + "."
...
def suggested_player_card_link(row: pd.Series, slot: str) -> str:
    """Clickable HTML card for the horizontal suggested-player rail."""
    player_name = str(row.get("Player", ""))
    href = f"?add_slot={quote(str(slot))}&add_player={quote(player_name)}"
    return f"""
    <a class="suggest-card-link" href="{href}">
...
def open_player_search_dialog(slot: str):
    ...
    if len(suggested_players) > 0:
        suggested_html = '<div class="suggest-rail">'
        for _, srow in suggested_players.iterrows():
            suggested_html += suggested_player_card_link(srow, slot)
        suggested_html += '</div>'
        st.markdown(suggested_html, unsafe_allow_html=True)
    else:
    ...
    # Card list replaces the old dropdown/selectbox. Every visible card has its own Add button.
    for idx, (_, prow) in enumerate(player_pool.head(12).iterrows()):
    ...
    if len(player_pool) > 12:
        st.caption("Showing top 12 matches. Use search or filters to narrow the list.")
