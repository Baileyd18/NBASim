...
DATA_PATH = "Clean NBA Data With Headshots.csv"
...
def load_data(path: str) -> pd.DataFrame:
    ...
    # Optional image fields. The app still works if these are missing,
    # but will show initials instead of player headshots.
...
try:
    ...
    try:
        ...
        st.warning(
            ...
            "Headshots will only show if that CSV has a HeadshotURL column."
        )
    except FileNotFoundError:
        st.error(
            f"Could not find `{DATA_PATH}` or `{FALLBACK_DATA_PATH}`. "
            "Put the app file in the same folder as your CSV."
        )
        st.stop()
...
.player-headshot {
    ...
    background: radial-gradient(circle at 30% 20%, rgba(56, 189, 248, .20), rgba(15, 23, 42, .95));
...
def player_headshot_html(row) -> str:
    """
    Returns a real NBA headshot when HeadshotURL is available.
    Falls back to initials automatically if the image URL is missing or fails.
    """
    try:
    ...
    if url and url.lower() not in ["nan", "none"]:
        return f"""
            ...
            />
            <div class="avatar-circle avatar-fallback">{initials_text}</div>
        """

    return f'<div class="avatar-circle">{initials_text}</div>'
