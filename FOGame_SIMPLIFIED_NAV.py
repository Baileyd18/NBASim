...
def suggested_player_card_html(row: pd.Series, slot: str) -> str:
    """HTML card used inside a Streamlit component. No anchor tag, so it will not open a new tab."""
    player_name = str(row.get("Player", ""))
    slot_js = json.dumps(str(slot))
    player_js = json.dumps(player_name)

    return f"""
    <button class="suggest-card" type="button" onclick='addSuggestedPlayer({slot_js}, {player_js})'>
...
def render_suggested_players_row(slot: str, suggested_players: pd.DataFrame):
    """
    Render suggestions as one horizontal mobile-friendly row.

    This uses a Streamlit component with real button elements and JavaScript.
    It does not use anchor links, because mobile Safari/Streamlit can open
    markdown links in a new tab. The JS redirects the current parent Streamlit
    page with query params, and process_query_param_add() adds the player.
    """
    if suggested_players is None or len(suggested_players) == 0:
        return

    cards_html = "".join(
        suggested_player_card_html(row, slot)
        for _, row in suggested_players.head(5).iterrows()
    )
    ...
    <style>
        ...
        .suggest-scroll-row::-webkit-scrollbar {{ height: 5px; }}
        .suggest-scroll-row::-webkit-scrollbar-thumb {{
        ...
        @media (max-width: 700px) {{
            .suggest-scroll-row {{ gap: 9px; padding-bottom: 12px; }}
            .suggest-card {{ flex-basis: 118px; width: 118px; min-width: 118px; max-width: 118px; height: 228px; min-height: 228px; padding: 9px 7px; }}
            .suggest-img-wrap img, .suggest-img-wrap .avatar-circle {{ width: 46px !important; height: 46px !important; }}
            .suggest-name {{ font-size: 12px; min-height: 32px; }}
        }}
    </style>
    </head>
    <body>
        <div class="suggest-scroll-row">{cards_html}</div>
        <script>
            function addSuggestedPlayer(slot, player) {{
                const params = new URLSearchParams();
                params.set('add_slot', slot);
                params.set('add_player', player);
                const parentLocation = window.parent.location;
                const newUrl = parentLocation.origin + parentLocation.pathname + '?' + params.toString();
