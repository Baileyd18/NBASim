...
@media (max-width: 700px) {
    ...
    .suggest-scroll-row .suggest-card { height: 198px !important; min-height: 198px !important; padding: 9px 7px !important; }
...
def suggested_player_card_link(row: pd.Series, slot: str) -> str:
    ...
    return (
        f'<a class="suggest-card-link" target="_self" href="{href}">'
...
def render_suggested_players_row(slot: str, suggested_players: pd.DataFrame):
    ...
    cards_html = "".join(
        suggested_player_card_link(row, slot)
        for _, row in suggested_players.head(5).iterrows()
    )

    st.markdown(
        f'<div class="suggest-scroll-row">{cards_html}</div>',
        unsafe_allow_html=True
    )
