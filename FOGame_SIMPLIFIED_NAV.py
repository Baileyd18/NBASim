...
def open_player_search_dialog(slot: str):
    ...
    st.markdown(
        ...
        <div class="dialog-player-card">
            <div class="avatar-circle">{initials(selected_player['Player'])}</div>
            <div>
...
def render_roster_list(show_remove: bool = False):
    ...
    for slot in active_roster_slots:
        if slot in st.session_state.roster:
            ...
            row = st.container()
            with row:
                c_slot, c_plus, c_info, c_right, c_chev = st.columns([0.55, 0.85, 4.9, 1.15, 0.25], gap="small")
                with c_slot:
                    st.markdown(f'<div class="slot-pill slot-pill-button-row">{slot_short(slot)}</div>', unsafe_allow_html=True)
                with c_plus:
                    if st.button("+", key=f"add_slot_{slot}", use_container_width=True, help=f"Add player to {slot}"):
                ...
                with c_right:
                    st.markdown(
                        """
                        <div class="player-right-mobile empty-right-block">
                ...
                with c_chev:
                    st.markdown('<div class="chev-mobile chev-button-row">›</div>', unsafe_allow_html=True)
