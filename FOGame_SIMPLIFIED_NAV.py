...
def open_player_search_dialog(slot: str):
    ...
    if len(suggested_players) > 0:
        # Native Streamlit buttons are used for Add actions.
        # This prevents mobile browsers from opening a new tab/window,
        # while the cards still stay in one compact horizontal row.
        suggested_players = suggested_players.head(5).reset_index(drop=True)
        suggested_cols = st.columns(len(suggested_players), gap="small")

        for idx, (_, srow) in enumerate(suggested_players.iterrows()):
            with suggested_cols[idx]:
                st.markdown(
                    f"""
                    <div class="suggest-card suggest-card-native">
                ...
                if st.button(
                    ...
                    key=f"suggest_add_{slot}_{idx}_{srow['Player']}"
                ):
